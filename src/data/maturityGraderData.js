...
export function getDimensionGrade(score) {
...
export function gradeColor(grade) {
  ...
  const map = { A: '#10B981', B: '#60A5FA', C: '#FBBF24', D: '#F97316', F: '#EF4444' };
  return map[base] || map.F;
}

/**
 * Compute full results from answers map.
 * @param {Object} answers — { q1: 25, q2: 8, ... } (questionId → points)
 * @returns {{ overall: { score, max, grade, label }, dimensions: Array }}
 */
export function computeResults(answers) {
  const dimResults = dimensions.map((dim) => {
    const score = dim.questions.reduce((sum, q) => sum + (answers[q.id] || 0), 0);
